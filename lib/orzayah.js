...
/* ============================================================
   ...
   l'API ORZAYAH qui :
     ...
     - lie le code au numéro de téléphone ORZAYAH du membre,
...
export function retirerChampsOrzayahServeur(values) {
  ...
  if ("orzayahTelephone" in values) values.orzayahTelephone = normaliserTelephone(values.orzayahTelephone) || null;
  return values;
}
...
export async function lierCompteOrzayah(table, row, { force = false } = {}) {
  ...
  const telephone = normaliserTelephone(row.orzayahTelephone);
  if (!telephone) {
    return enregistrer({ orzayahCompte: code, orzayahStatut: "erreur", orzayahErreur: "Numéro de téléphone ORZAYAH requis : c'est le numéro sur lequel le compte est créé." });
  }
